module AutoCompleteMacrosHelper      
  ...
  # auto complete untuk multiple text box
  # tambah action multi_auto_complete_for di controller
  # edit oleh Rijaludin Muhsin
  # 

  def multi_text_field_with_auto_complete(object, method, index, tag_options = {}, completion_options = {})
    (completion_options[:skip_style] ? "" : auto_complete_stylesheet) +
    text_field_tag("#{object}_#{method}_#{index}", "", tag_options) +
    content_tag("div", "", :id => "#{object}_#{method}_#{index}_auto_complete", :class => "auto_complete") +
    auto_complete_field("#{object}_#{method}_#{index}", { :url => { :action => "multi_auto_complete_for_#{object}_#{method}", :index => index } }.update(completion_options))
  end
