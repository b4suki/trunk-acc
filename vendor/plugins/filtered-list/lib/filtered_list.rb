...
module FilteredList
  ...
  module FilterFind
    ...
    def filter_find(filter, *args)
      return find(*args) if filter.nil? || filter.empty? 
  ...
  module Helper
    ...
    def filter(collection, fld, options={}, html_options={}, &blk)
      ...
      cur = params[options[:prefix]].nil? ? ':all' : params[options[:prefix]][fld]
      cur = '' if cur == ':all'
    ...
    def filtered_header(object, method)
      filter object, method.to_sym do |local|
        [local.send(method), local.send(method)]
      end 
    end
  end
end
