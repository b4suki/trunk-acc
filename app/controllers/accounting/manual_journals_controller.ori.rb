class Accounting::ManualJournalsController < ApplicationController
  ...
  def filter_date 
    ...
    render :update do |page|
      ...
      page.replace "option_debit", :partial => "option_debit", :locals => {:result => @result, :date => @date}
      page.replace "option_credit", :partial => "option_credit", :locals => {:result => @result, :date => @date}
  ...
  def account_filter_purchase(value, conditions_debit)
    ...
    if value != "all"  && conditions_debit.nil?
      ...
      account_purchase_id = AccountingPurchaseDebitCreditValue.find_by_value($SELECTED_DEBIT).accounting_purchase_debit_credit.account_id
  ...
  #   p $CONDITIONS_DESCRIPTION
  #   p "aaa"
  #   p "-----------"
  #   p split_conditions($CONDITIONS_DESCRIPTION.split(" LIKE "), "DESCRIPTION")
  ...
  def edit_kas
    @cash = AccountingCashBalance.find(params[:id])   
    render :layout => false
  end
  ...
  def edit_kontra_bank
    @cash = AccountingBankBalance.find(params[:id])
    render :layout => false
  end
  
  def edit_bank
    @bank = AccountingBankBalance.find(params[:id])
    render :layout => false
  end
  ...
  def update_kas    
    ...
    respond_to do |format|
      if @cash.update_attribute(:account_id,params[:cash][:account_id])
        flash[:notice] = 'Data Kode rekening diupdate.'
        format.html { redirect_to(accounting_manual_journals_path) }
        format.xml  { head :ok }
      else
        flash[:notice] = 'Data Kode rekening gagal diupdate.'
  ...
  def update_kontra_kas
    @cash = AccountingCashBalance.find(params[:id])

    respond_to do |format|
      if @cash.update_attribute(:contra_account_id,params[:cash][:account_id])
        flash[:notice] = 'Data Kode rekening diupdate.'
        format.html { redirect_to(accounting_manual_journals_path) }
        format.xml  { head :ok }
      else
  ...
  def update_bank
    ...
    respond_to do |format|
      if @bank.update_attribute(:account_id,params[:cash][:account_id])
        flash[:notice] = 'Data Kode rekening diupdate.'
        format.html { redirect_to(accounting_manual_journals_path) }
        format.xml  { head :ok }
      else
  ...
  def update_kontra_bank
    @bank = AccountingBankBalance.find(params[:id])

    respond_to do |format|
      if @bank.update_attribute(:contra_account_id,params[:bank][:account_id])
        flash[:notice] = 'Data Kode rekening diupdate.'
        format.html { redirect_to(accounting_manual_journals_path) }
        format.xml  { head :ok }
      else
  ...
  def prepare_show_summary(result, account, debit, credit)
    ...
    result.each do |journal|
      ...
      summary_credit = summary_credit + summary[0]
      summary_debet = summary_debet + summary[1]
    end    
    ...
    if journal.class == AccountingCashBalance
      if(!debit && !credit)
        if (journal.account_id == account[:value].to_i)
          ret_debet += journal.transaction_type_id == 1 ? journal.transaction_value : 0
          ret_kredit  += journal.transaction_type_id != 1 ? journal.transaction_value : 0
          if journal.contra_account
            if ( journal.contra_account_id == account[:value].to_i)
              ret_debet = ret_debet + journal.transaction_type_id == 1 ? 0 :journal.transaction_value
              ret_kredit = ret_kredit + journal.transaction_type_id != 1 ? 0 : journal.transaction_value
            end
          end
        end
      end
  ...
  def summary_general_journal_from_bank_balance(journal, account, debit, credit)
    ...
    if journal.class == AccountingBankBalance
      if(!debit && !credit)
        if( journal.account_id == account[:value].to_i)
          ret_debet += journal.debit ? journal.transaction_value : 0
          ret_kredit += journal.credit ? journal.transaction_value : 0
          if journal.contra_account
            if (journal.accounting_account.id == account[:value].to_i)
              ret_debet += journal.debit ? 0 : journal.transaction_value
              ret_kredit += journal.credit ? 0 : journal.transaction_value
            end
          end
        end
      end
      ...
      if(!debit && !credit)
        ...
        journal.values.each do |value|         
          if value.account_id == account[:value].to_i
            if value.accounting_sale_debit_credit.debit              
      ...
      if(!debit && !credit)
        journal.values.each do |value|
          if value.accounting_purchase_debit_credit.account_id == account[:value].to_i            
            if value.accounting_purchase_debit_credit.debit == true
              ret_debet += value.value
            elsif value.accounting_purchase_debit_credit.credit == true
              ret_kredit += value.value
            end
          end
        end
  ...
  def get_first_saldo(group, previous_date)
